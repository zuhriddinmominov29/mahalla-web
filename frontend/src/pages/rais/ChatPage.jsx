...
export default function RaisChatPage() {
  ...
  useEffect(() => {
    loadMessages();
    const channel = supabase.channel('messages-rais')
  ...
  async function sendMessage(e) {
    ...
    setMessages(prev => [...prev, {
      id: tempId, content: savedContent, created_at: new Date().toISOString(),
    ...
    try {
      ...
      if (loc) { form.append('latitude', loc.latitude); form.append('longitude', loc.longitude); form.append('accuracy', loc.accuracy); }
  ...
  async function deleteMessage(id) {
    ...
    if (!res.success) { toast.error(res.message || "O'chirib bo'lmadi"); loadMessages(); }
    else toast.success("Xabar o'chirildi");
  }
  ...
  async function saveEdit(id) {
    ...
    } else toast.error(res.message || 'Xato');
  }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-gray-950">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {loading ? (
          <div className="flex justify-center items-center h-full"><div className="text-gray-400">Yuklanmoqda...</div></div>
        ) : messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <span className="text-5xl mb-3">💬</span>
            <p className="text-gray-500">Hali hisobot yo'q</p>
            <p className="text-gray-400 text-sm">Bugungi hisobotingizni yuboring</p>
          </div>
        ) : messages.map(msg => (
          <div key={msg.id} className={`flex ${isMine(msg) ? 'justify-end' : 'justify-start'} group`}>
            <div className={`max-w-[75%] ${isMine(msg) ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
              {!isMine(msg) && (
                <span className="text-xs text-gold-600 dark:text-gold-400 font-medium px-1">
              ...
              <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed
                ...
                ${isMine(msg)
                  ...
                  : isFromHokim(msg)
                  ? 'bg-gold-50 dark:bg-gold-500/10 border border-gold-200 dark:border-gold-500/20 text-gray-800 dark:text-gray-100 rounded-bl-sm'
                  : 'bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-gray-800 dark:text-gray-100 rounded-bl-sm'}`}>
                ...
                {editingId === msg.id ? (
                  <div className="flex flex-col gap-2 min-w-48">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => saveEdit(msg.id)} className="flex-1 bg-primary-600 text-white text-xs py-1.5 rounded-lg font-semibold">Saqlash</button>
                      <button onClick={() => setEditingId(null)} className="flex-1 bg-slate-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs py-1.5 rounded-lg">Bekor</button>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    {msg.attachments?.map(a => (
                      <div key={a.id} className="mt-2">
                        {a.mime_type?.startsWith('image/') ? (
                          <img src={a.url} alt={a.name} className="max-w-xs rounded-xl cursor-pointer hover:opacity-90" onClick={() => window.open(a.url, '_blank')} />
                        ) : (
                          <a href={a.url} target="_blank" rel="noopener"
                            className="flex items-center gap-2 bg-slate-100 dark:bg-gray-900/50 rounded-lg px-3 py-2">
                ...
                {isMine(msg) && !msg._sending && (
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setEditingId(msg.id); setEditText(msg.content || ''); }}
                      className="text-xs px-1.5 py-0.5 rounded-md bg-slate-200 dark:bg-gray-800 text-gray-500 hover:text-gray-800 dark:hover:text-white transition-all">✏️</button>
                    <button onClick={() => deleteMessage(msg.id)}
                      className="text-xs px-1.5 py-0.5 rounded-md bg-slate-200 dark:bg-gray-800 text-gray-500 hover:text-red-500 transition-all">🗑️</button>
                  </div>
                )}
              </div>
            </div>
      ...
      {/* File previews */}
      {files.length > 0 && (
        <div className="px-4 py-2 bg-white dark:bg-gray-900 border-t border-slate-200 dark:border-gray-800 flex gap-2 flex-wrap">
          {files.map((f, i) => (
      ...
      <div className="bg-white dark:bg-gray-900 border-t border-slate-200 dark:border-gray-800 p-4">
        <form onSubmit={sendMessage} className="flex gap-3 items-end">
          <button type="button" onClick={() => fileRef.current?.click()}
            className="w-10 h-10 flex-shrink-0 bg-slate-100 dark:bg-gray-800 hover:bg-slate-200 dark:hover:bg-gray-700 rounded-xl flex items-center justify-center text-gray-500 hover:text-gray-800 dark:hover:text-white transition-all">📎</button>
          <input ref={fileRef} type="file" multiple className="hidden" onChange={handleFiles} accept="image/*,.pdf,.doc,.docx,.xls,.xlsx" />
          <textarea ref={textareaRef} value={content} onChange={e => setContent(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(e); }}}
            placeholder="Hisobotingizni yozing... (Enter = yuborish)"
            rows={1} className="input flex-1 resize-none py-2.5"
