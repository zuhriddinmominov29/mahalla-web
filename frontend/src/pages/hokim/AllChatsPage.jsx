...
export default function HokimAllChatsPage() {
  ...
  const [selected,  setSelected]  = useState(null); // null = hammasi
  const [content,   setContent]   = useState('');
  const [replyTo,   setReplyTo]   = useState('all'); // 'all' | userId[]
  ...
  useEffect(() => {
    api.get('/users').then(r => { if (r.success) setUsers(r.data.filter(u => ['rais','uyushma'].includes(u.role))); });
  }, []);
      ...
      if (res.success) {
        const msgs = selected
          ? res.data.filter(m => m.sender?.id === selected ||
              (m.sender?.role === 'hokim' &&
               (m.recipients === null || m.recipients?.includes(selected))))
          : res.data;
        setMessages(msgs);
      }
    } finally {
  ...
  async function sendReply(e) {
    ...
    try {
      ...
      // recipients
      if (replyTo !== 'all') {
        const ids = replyTo === 'selected' && selected ? [selected] : [];
        if (ids.length) form.append('recipients', JSON.stringify(ids));
      }
    ...
    <div className="flex h-full">
      ...
      <div className="w-72 bg-gray-900 border-r border-gray-800 flex flex-col">
        <div className="p-4 border-b border-gray-800">
          <h2 className="font-semibold text-white text-sm">Raislar ({users.length})</h2>
        </div>
        <div className="flex-1 overflow-y-auto">
          {/* Hammasi */}
          <button onClick={() => setSelected(null)}
            className={`w-full flex items-center gap-3 px-4 py-3 text-sm text-left hover:bg-gray-800 transition-all
              ${!selected ? 'bg-gray-800 border-r-2 border-primary-500' : ''}`}>
            <div className="w-9 h-9 bg-primary-600/20 rounded-xl flex items-center justify-center text-lg flex-shrink-0">💬</div>
            <div>
              <div className="font-medium text-gray-200">Barcha xabarlar</div>
          ...
          {users.map(u => {
            ...
            return (
              <button key={u.id} onClick={() => setSelected(u.id)}
                className={`w-full flex items-center gap-3 px-4 py-3 text-sm text-left hover:bg-gray-800 transition-all
                  ${selected === u.id ? 'bg-gray-800 border-r-2 border-primary-500' : ''}`}>
                <div className="w-9 h-9 bg-gray-700 rounded-xl flex items-center justify-center text-base flex-shrink-0">
                  {u.role === 'uyushma' ? '🤝' : '👤'}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-gray-200 truncate text-xs">{u.full_name}</div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-gray-900 border-b border-gray-800 px-5 py-4 flex items-center gap-3">
          {selected ? (
            <>
              ...
              <div>
                <div className="font-semibold text-white text-sm">{selectedUser?.full_name}</div>
                <div className="text-xs text-gray-400">{selectedUser?.mahallas?.name} MFY</div>
              </div>
            </>
          ) : (
            <>
              <div className="w-9 h-9 bg-primary-600/20 rounded-xl flex items-center justify-center">💬</div>
              <div>
                <div className="font-semibold text-white text-sm">Barcha xabarlar</div>
                <div className="text-xs text-gray-400">{messages.length} ta xabar</div>
              </div>
            </>
          )}
            ...
            {['all','selected'].map(r => (
              <button key={r} onClick={() => setReplyTo(r)}
                className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-all
                  ${replyTo === r ? 'bg-gold-500 text-gray-950 border-gold-500' : 'border-gray-700 text-gray-400 hover:border-gray-600'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {loadingMsg ? (
            <div className="flex justify-center py-8 text-gray-500">Yuklanmoqda...</div>
          ) : messages.length === 0 ? (
              ...
              <div key={msg.id} className={`flex ${isHokim ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[70%] flex flex-col gap-1 ${isHokim ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`rounded-2xl px-4 py-3 text-sm
                    ${isHokim ? 'bg-gold-500/10 border border-gold-500/20 text-gray-100 rounded-br-sm'
        ...
        <div className="bg-gray-900 border-t border-gray-800 p-4">
          ...
          <form onSubmit={sendReply} className="flex gap-3 items-end">
            <button type="button" onClick={() => fileRef.current?.click()}
              className="w-10 h-10 flex-shrink-0 bg-gray-800 hover:bg-gray-700 rounded-xl flex items-center justify-center text-gray-400 hover:text-white transition-all">
              📎
            </button>
